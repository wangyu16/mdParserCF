 ...
 * - Single line: everything after `key: ` is the value
 * - Multiline: use `key: |` syntax
 *
...
export class NymlParseError extends Error {
  code: string;
  line: number;
  column: number | null;
 ...
 *
 * @param stripped - Line with leading/trailing whitespace removed
 * @returns [key, value] or [null, null] if no colon
 */
 ...
 * @param startIdx - Index of the "key: |" line
 * @param baseIndent - Indent of the key line
...
// Type for NYML values
export type NymlValue = string | NymlEntry | NymlValue[];
export type NymlEntry = { [key: string]: NymlValue };

/**
 * Parse NYML V2 text into a list structure.
 *
 * @param text - The NYML V2 content to parse
 * @returns A list representing the parsed data
 */
export function parseNyml(text: string): NymlValue[] {
  const lines = text.split(/\r?\n/);
  const result: NymlValue[] = [];

  // Stack of [container, baseIndent]
  const stack: [NymlValue[], number][] = [[result, -1]];
  ...
  while (i < lines.length) {
    ...
    // Single-line key:value (value is guaranteed non-null here since key is non-null)
    parent.push({ [key]: value as string });
    i++;
  }
 ...
 *
 * @param data - List structure as returned by parseNyml()
 * @param indent - Current indentation level (spaces)
 * @returns NYML formatted string
 */
export function serializeNyml(data: NymlValue[], indent: number = 0): string {
  const lines: string[] = [];
    ...
    if (typeof item === 'string') {
      ...
      for (const [key, value] of Object.entries(item)) {
        ...
        if (Array.isArray(value)) {
          ...
          lines.push(serializeNyml(value, indent + 2));
